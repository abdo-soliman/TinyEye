...
void svm::fit(torch::Tensor dataset, torch::Tensor labels, bool log, int iterations)
{
    ...
    for (int epoch = 0; epoch < iterations; epoch++)
    {
        ...
        if (log)
            logger::LOG_INFO("Epoch: " + std::to_string(epoch) + ", Loss: " + std::to_string(sum_loss / N) + ", Acc: " + std::to_string(acc / N));
    }
    final_acc = acc / N;
    trained = true;
}
...
torch::Tensor svm::predict_many(torch::Tensor embeddings)
{
    this->eval();
    auto output = forward(embeddings);
    return output.argmax(1).to(torch::kLong);
}
