...
MTCNN::MTCNN(mtcnn_config config)
{
	#ifdef CPU_ONLY
	...
	// load the networks
	p_net.reset(new caffe::Net<float>((config.models_dir + "/pnet.prototxt"), caffe::TEST));
	p_net->CopyTrainedLayersFrom(config.models_dir + "/pnet.caffemodel");
	...
	r_net.reset(new caffe::Net<float>((config.models_dir + "/rnet.prototxt"), caffe::TEST));
	r_net->CopyTrainedLayersFrom(config.models_dir + "/rnet.caffemodel");
	...
	{
		steps_threshold[0] = config.thresholds[0];
		steps_threshold[1] = config.thresholds[1];
	}

	min_face_size = config.min_fs;
	scale_factor = config.sf;
}

MTCNN::MTCNN(std::string models_dir, float thresholds[2], uint32_t min_fs, double sf)
{
	#ifdef CPU_ONLY
		caffe::Caffe::set_mode(caffe::Caffe::CPU);
	#else
	...
	{
		steps_threshold[0] = thresholds[0];
		steps_threshold[1] = thresholds[1];
	}
